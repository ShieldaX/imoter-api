...
const demo_album_uris = [
  ...
  'https://www.nvshens.net/g/30432/', // 已删除的页面
...
const parseAlbum = async (opts, albumId) => {
  ...
  return requesto(options)
    .then(($) => {
      ...
      let isNormal = util.isUndefined(first_img_src)
      if (isNormal) {
        return album;
      }
      ...
      }else if (moters_num < 1) { //对应没有模特标签的情况，只能根据首张照片地址来确定
        console.log('No moter tag find, use first images moter id!')
        album.moters.push(moterId);
      };
      // console.log(album.moters);
      //图集简介expert/bio
    ...
    .finally(async () => {
      ...
      if (!_exists) {
        ...
        if (album.title.length != 0 && album.moters.length != 0 ) {
          let _album = await Album.create(album);
          console.log(_album);
        } else {
          console.log(`未能正确解析该图集资源：${album._id}`); // TODO: Log this error to check manually later
        }
      } else {
