...
const instURIs = [
  ...
  'https://www.nvshens.net/girl/17219/', //小青蛙
      ...
      $('td.info_td', info).each(function(index, element) {
        ...
        switch (field){
          ...
          case '三 围：':
            ...
            let cup = value.match(/\(([A-K]\d*)/);
            if (cup) {moter.cup = cup[1]};
            let measureStr = value.replace(/\([A-K]+\d*\)/, '');
            let measured = measureStr.match(/B(?<bust>\d+)\sW(?<waist>\d+)\sH(?<hips>\d+)/);
            let measureG;
            if (measured) {
              measureG = measured.groups;
            } else {
              measureG = {};
              measureG.bust = measureStr.match(/B(?<bust>\d+)/) ? measureStr.match(/B(?<bust>\d+)/).groups.bust : null;
              measureG.waist = measureStr.match(/W(?<waist>\d+)/) ? measureStr.match(/W(?<waist>\d+)/).groups.waist : null;
              measureG.hips = measureStr.match(/H(?<hips>\d+)/) ? measureStr.match(/H(?<hips>\d+)/).groups.hips : null;
            }
            moter.measure = {};
