...
async function* seriesAlbumParser(start, direction, limit) {
	// ensure limitation
	let ending, step;
	if (direction) {
		ending = start - limit;
	...
	try {
		while (start != ending) {
			let album = await parse_album(String(start));
			// console.log(album);
			start = start + step;
			await Delay_Time(3000, true);
			yield album;
		};
	} catch (err) {
		console.log(err);
...
const parse_seriously = async () => {
  let albums = [];
  for await (let album of seriesAlbumParser(current_album_id, directions.down, 2)) {
    albums.push(album);
    console.log(album);
  };
  // console.log(albums);
