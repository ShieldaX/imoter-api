...
const Delay_Time = function(ms, flag) {
	if (flag) {
...
async function* seriesAlbumParser(album_ids) {
	try {
		while (album_ids.length > 0) {
			let album = await parse_album(album_ids[0]);
...
const parse_seriously = async (album_ids) => {
  let albums = [];
  for await (let album of seriesAlbumParser(album_ids)) {
    albums.push(album);
    console.log(album);
  };
  // console.log(albums);
...
const fetch_albumlist_page = async (page_url) => {
  if (!page_url) {
...
async function* seriesAlbumPageParser(urls) {
  try {
    while (urls.length > 0) {
      let page = await fetch_albumlist_page(urls[0]);
      // 从任务列表中移除已经解析完成的图集任务id
      urls.splice(0, 1);
      yield page;
      await Delay_Time(20000, true);
    };
  } catch (err) {
    console.log(err);
  } finally {
...
const spawn = async (pages) => {
  let apages = [];
  for await (let page of seriesAlbumPageParser(pages)) {
    apages.push(parse_seriously(page));
  };
  console.log(`${apages.length} Pages Done!`);
};

const fetch_album_page_urls = async (page_url) => {
  if (!page_url) {
    page_url = 'https://www.nvshens.com/gallery/'
  }
  let cur_host = 'https://'+url.parse(page_url).host;
  let pages = [];
  getPage(page_url)
  .then(($) => {
    let begin = false;
  ...
  .finally(async ()=>{
    await spawn(pages);
    fetch_album_page_urls(pages[pages.length-1]);
  })
};

fetch_album_page_urls();