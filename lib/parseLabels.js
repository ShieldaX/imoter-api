...
async function parsePages(spec_options, label, page_num) {
  ...
  let options = {uri: 'https://'+cur_host+'/tag/'+label._id+((page_num>1) ? '/'+page_num+'.html' : '')};
  options.transform = spec_options.transform;
  options.timeout = 10000;
  let interval = Number(page_num)*1000;
  console.log("Delay Time "+interval);
  let finaltag;
  if (page_num > 0) {
    try{
      // 使用await关键字确保每次抓完再抓列表中的下一个
      finaltag = await parsePage(options, label, page_num);
      page_num = page_num - 1;
      // 设置间隔时间
...
async function parseTag(spec_options, label){
  ...
  .then(async ($) => {
    ...
    $('#listdiv').find('li.beautyli strong a').each((index, element) => {
      let tag = $(element).attr('href').split('/')[2];
      label.moters.push(tag);
    });
