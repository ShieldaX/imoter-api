...
const instUris = [
  ...
  'https://www.nvshens.net/girl/17565/gallery', //曾莉婷(只有一个图集的)
  'https://www.nvshens.net/girl/16416/gallery' //王心恬
];
...
const demo_album_uris = [
  'https://www.nvshens.net/girl/17565/gallery', //木木夕Mmx(缺资料)
...
const parseGalleryPages = async (opts, gallery, interval) => {
  ...
  await requesto(opts)
    .then(async ($) => {
      ...
      if (next_page.text().match('后')) {
        ...
        console.log(`解析下一页: ${opts.uri}`);
        await wait(interval);
        await parseGalleryPages(opts, gallery, interval);
      };
...
// 解析指定模特的相册
const parseGallery = async (opts, moter_id, interval) => {
  interval = interval || 5000;
  // 构建相册模型
  let gallery = {};
  var cur_host = url.parse(opts.uri).host;
  if (moter_id) { // URI overwrite by given moter_id
    opts.uri = 'https://'+cur_host+'/girl/'+moter_id+'/gallery';
    console.log('Url Overwrite To: '+opts.uri);
  };
  gallery.moter = opts.uri.match(moterRegex)[1];
