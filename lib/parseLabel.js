...
async function parsePage(page){
  ...
  return requesto(page)
  .then(($)=>{
    // let moter_id = $('#listdiv').find('li.beautyli a').attr('href').match(moterRegex)[1];
    let moter_id = $('#listdiv').find('li.beautyli').length;
    console.log(moter_id);
  });
};

// 依次解析任务列表中的Label
async function parsePages(pages, interval){
  let finaltag;
  if (pages.length > 0) {
    try{
      await Delay_Time(interval);
      // 使用await关键字确保每次抓完再抓列表中的下一个
      finaltag = await parsePage(pages[0], interval);
      ...
      await Delay_Time(interval);
      // 继续抓取列表中剩余的Label
      return parsePages(pages, interval);
    }catch(err){
      console.log(err);
    };
  }else{
    return console.log("Pages All Done");
  };
};

// 解析出Label对应的详情介绍并返回结果(以备保存)
async function parseLabel(spec_options, tag, interval){
  ...
  .finally(()=>{
    ...
    // console.log(pages);
    parsePages(pages, interval);
  });
  // return tag;
};

options.uri = 'https://www.nvshens.com/tag/yanyuan'
parseLabel(options, {_id: 'xiezhen'});
...
async function parseLabels(spec_options, tags, interval){
  ...
  if (tags.length > 0) {
    try{
      await Delay_Time(interval);
      // 使用await关键字确保每次抓完再抓列表中的下一个
      finaltag = await parseLabel(spec_options, tags[0], interval);
      // console.log(finaltag);
      // let tag = await Label.create(finaltag);
      // console.log(tag);
      // 从原始列表中取出已经解析完成的Label
