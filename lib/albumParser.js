...
const demo_album_uris = [
  ...
  'https://www.nvshens.com/g/29604/', // 合辑
  'https://www.nvshens.com/g/19198/' //多人合辑，所属不确定
];
...
const parseAlbum = async (spec_options, album_id) => {
  ...
  album_id = album_id || spec_options.uri.match(albumRegex)[1];
  ...
  return requesto(options)
    .then(($) => {
      ...
      let moter_id = first_img_src.match(galleryRegex)[1];
      // 包含两个以上模特的确保所属moter_id在数组最后
      let moters_num = album.moters.length;
      if(moters_num > 1 && _(album.moters).indexOf(moter_id) < (moters_num-1)) {
        let _moters = _(album.moters).without(moter_id);
        _moters.push(moter_id);
...
const parseAlbumPage = async () => {
  let album_ids = [];
  getPage('https://www.nvshens.com/gallery/')
  .then($ => {
    let list_selector = '#listdiv ul li';
    $(list_selector).each(function(index, el) {
      let link=$('a.galleryli_link', el).attr('href');
      album_ids.push(link.match(/\d+/)[0]);
    });
    return album_ids.map(parseAlbum)
      .reduce()
